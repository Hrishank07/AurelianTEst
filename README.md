...
## Things I Did - Hrishank

As a developer reviewing the starter code, my primary goal was to integrate the new features seamlessly into the existing architecture rather than over-engineering a new one. Here is a breakdown of what I implemented and why:

### Task 1 & 2: Core Logic & API

* **Fixing the Boilerplate:** While implementing `update_chat`, I noticed the starter code’s tool execution logic didn't correctly handle dynamic function mapping—it was actually crashing when trying to run the tool. I refactored this logic to safely route `submit`, `update`, and `delete` actions, adding error handling to prevent the agent from crashing if a tool call fails.
* **Modernizing the Stack:** I noticed the Pydantic schemas were using the deprecated `orm_mode`. I proactively updated all models to use the Pydantic V2 syntax (`from_attributes = True`). This ensures the application is forward-compatible and keeps the logs clean of runtime warnings.
* **Defensive API Design:** For the new manual `PUT` routes, I added strict server-side validation. The endpoint explicitly checks that the status is a valid enum (1, 2, or 3) and returns a helpful `400 Bad Request` if the payload is invalid, rather than letting the server throw a generic 500 error.
* **Leveraging Existing Patterns:** I stuck to the existing pattern of using `crud.py` for database operations. It would have been easy to write raw SQL or add a new service layer, but keeping the codebase consistent makes it much easier for the rest of the team to maintain.

### Task 3: Change History Design

* **Polymorphic Audit Log:** For the revision history, I designed a generic, polymorphic `AuditLog` model (detailed in `task3.md`). Instead of creating a tightly coupled `FormHistory` table that takes snapshots of entire rows, I opted for an `entity_type` + `entity_id` field-level tracking system. This is a pattern I’ve seen work well in production because it allows us to track changes across *any* future model (Chats, Users, etc.) without needing to run new database migrations every time we add a feature, something similar I did in prev work.
